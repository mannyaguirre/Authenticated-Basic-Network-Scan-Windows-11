# Authenticated Basic Network Scan: Windows 11 VM
...
The purpose of this lab was to perform an **Authenticated Basic Network Scan**. An authenticated scan uses credentials to assess a target system  and discover any possible vulnerabilties more deeply than an unauthenticated scan. This allows the scanner to identify potential weaknesses that may not be visible without credentials.

In this lab I provisioned a Windows 11 virtual machine, created a Network Security Group (NSG), associated the NSG with our virtual machine, created an inbound rule in our NSG to allow connections from the Tenable Scanner, created an inbound network traffic rule for Windows firewall in the virtual machine, created the Authenticated Basic Network Scan, reviewed the results from the scan, deleted the virtual machine. 
...
- Create Authenticated Basic Network Scan
...
The virtual machine will serve as our endpoint for the Authenticated Basic Network Scan. 
...
### Step 4: Create Authenticated Basic Network Scan

The fourth step was to create Authenticated Basic Network Scan.
...
6. Credentials - Windows 11 VM Credentials ***Exbihit 13***
...
<img width="1918" height="980" alt="lab12" src="https://github.com/user-attachments/assets/2fa1b5f6-d269-493a-8d09-808e67368e94" />

### Exhibit 13

<img width="1918" height="980" alt="lab13" src="https://github.com/user-attachments/assets/75472176-7af3-4436-b434-e5060be1a61f" />
...
The Authenticated Basic Network Scan was finished in approximately 22 minutes taking double the time in comparison to the unauthenticated scan. This is due to the depth of the scan. In addition, the Authenticated Scan found more than double the number of vulnerabilities in comparison to the unauthenticated scan   ***Exibit 14***. The results showed the vulnerabilities that were found ***Exibit 15*** , the description for each vulnerability, the affected assets, the risk information such as CVSS scores, the solution to remediate the vulnerability ***Exhibit 16***  and an optional executive PDF summary ***Exhibit 17***.

### Exhibit 14

<img width="1918" height="922" alt="lab14" src="https://github.com/user-attachments/assets/d4e2bc2b-5ab7-40ec-805d-efb8165fb9af" />


### Exhibit 15

<img width="1918" height="913" alt="lab15" src="https://github.com/user-attachments/assets/9f43a6d1-e7bb-416f-aa60-8343eb21b672" />


### Exhibit 16

<img width="1918" height="912" alt="lab16" src="https://github.com/user-attachments/assets/c21efef0-f2cf-4b22-ad2b-33b64091f495" />


### Exhibit 17

<img width="862" height="682" alt="lab17" src="https://github.com/user-attachments/assets/f9d172a6-13e3-4c8d-813b-261fc68f030b" />
...
- Credentials are needed to perform a Authenticated Basic Network Scan.
- Create custom inbound network traffic rules to minimize incomplete scans.
- Authenticated Basic Network Scan is more detailed as a Unauthenticated Basic Network Scan.
...
In this lab I successfully completed an Authenticated Basic Network Scan, created custom NSG and  Windows Firewall inbound rules to allow network traffic from Tenable and reviewed the results from the scan.

This lab help me enforce my understanding for Authenticated Basic Network Scan, NSG configurations, firewall inbound rule configurations and review scan results.  
